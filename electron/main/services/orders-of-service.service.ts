...
class OrdersOfServiceService {
   ...
   ): Promise<OrderOfService[]> {
      const where: SQL[] = []
      where.push(eq(schema.ordersOfService.active, true))

      if (filters) {
         if (filters.customerName)
            where.push(like(schema.customers.name, `%${filters.customerName}%`))

         if (filters.vehicleName)
            where.push(like(schema.vehicles.model, `%${filters.vehicleName}%`))
      }

      return (await this._database.query.ordersOfService.findMany({
         columns: {
            id: true,
            date: true,
            totalValue: true,
